...
total=$(lshw -class network | awk '/logical name:/{print $3}')
declare -a total1=($total)
#In order to process individual work on the array, a loop was created.
for i in "${total1[@]}"; do
    if [[ $i = lo* ]] ;
	then continue ; fi
	#Conditions were applied to skip the loopback interface
   ...
   #To insert command output to the variable, normal lines are included.
  ...
  #Echo was done to print each and everything
